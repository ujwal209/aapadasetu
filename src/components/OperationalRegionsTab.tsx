...
export const OperationalRegionsTab: React.FC<OperationalRegionsTabProps> = ({
  ...
  return (
    <div className="space-y-4 text-white font-sans">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-[11px] font-mono tracking-wider text-neutral-400">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-neutral-300 uppercase">Operational Sectors</span>
          </div>
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-[10px] font-mono border transition cursor-pointer flex items-center space-x-1.5 ${
              isContinentalMode 
                ...
                : 'text-neutral-400 hover:text-white bg-neutral-900/90 border-neutral-800'
            }`}
          >
          ...
          {INDIA_DISASTER_ZONES.map((zone) => {
            ...
            return (
              <button
                ...
                style={{
                  borderColor: isSelected ? zone.color : 'rgba(38, 38, 38, 0.8)',
                }}
                className={`p-2.5 rounded-xl text-left transition duration-150 border cursor-pointer flex flex-col justify-between ${
                  isSelected
                    ? 'bg-neutral-900 text-white shadow-md'
                    : 'bg-neutral-950/90 hover:bg-neutral-900/60 text-neutral-400 hover:text-neutral-200'
                }`}
              >
                ...
                <div className="flex items-center justify-between mt-1.5 text-[10px] font-mono">
                  ...
                  <span className={`px-1.5 py-0.2 rounded-md ${isSelected ? 'bg-white text-black font-bold' : 'bg-neutral-900 text-neutral-400'}`}>
                    {count}
                  </span>
                </div>
              </button>
      ...
      {isScanningSector && (
        <div className="p-3 rounded-xl bg-neutral-950/90 border border-neutral-800 flex items-center space-x-2.5 text-neutral-300 text-xs animate-pulse">
      ...
      {!isContinentalMode ? (
        <div 
          ...
          className="rounded-2xl bg-neutral-950/95 border shadow-2xl overflow-hidden transition-all duration-200"
        >
          {/* Dossier Header Bar */}
          <div className="p-4 border-b border-neutral-900 flex items-center justify-between">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-[10px] font-mono font-bold uppercase tracking-wider bg-white text-black hover:bg-neutral-200 transition cursor-pointer flex items-center space-x-1.5 shrink-0 shadow-xs"
          ...
          <div className="divide-y divide-neutral-900 text-xs">
            ...
            <div className="p-4 space-y-2">
              <span className="text-[10px] font-mono uppercase tracking-wider text-neutral-400 block font-semibold flex items-center space-x-1.5">
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedZone.keyBasins.map((basin, idx) => (
                  <span
                    key={idx}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-mono bg-neutral-900 border border-neutral-800 text-neutral-200 font-medium"
                  >
                    {basin}
                  </span>
                ))}
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedZone.primaryThreats.map((threat, idx) => (
                  <span
                    key={idx}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-mono bg-neutral-900 border border-neutral-800 text-neutral-300 font-semibold"
                  >
                    {threat}
                  </span>
                ))}
            ...
            <div className="p-4 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-medium text-neutral-300">Active Verified Hazards (Past 7 Days)</span>
              </div>
              <span className="text-xs font-mono font-bold text-white px-2.5 py-1 rounded-lg bg-neutral-900 border border-neutral-800">
        ...
        <div className="p-4 rounded-2xl bg-neutral-950/95 border border-neutral-800 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Globe2 className="w-5 h-5 text-white" />
            <div>
              <h4 className="text-xs font-bold text-white">Full Continental Surveillance</h4>
              <p className="text-[10px] text-neutral-400">Simultaneous monitoring across all 5 disaster operational commands</p>
            </div>
          </div>
          <span className="text-xs font-mono font-bold px-2.5 py-1 rounded-lg bg-white text-black">
